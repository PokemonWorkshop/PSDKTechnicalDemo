...
begin
  ...
  origin << [:set_src_rect, 0, 0, 64 * 27]
  origin << [:waitcounter, total_wait += movement_wait]
  origin << [:se_play, 'hitlow', 60, 150] # Bruitage capture
  origin << [:set_src_rect, 0, 0, 64 * 28]
  origin << [:waitcounter, total_wait += movement_wait]
  origin << [:set_src_rect, 0, 0, 64 * 29]
  origin << [:waitcounter, total_wait += movement_wait]
  origin << [:set_src_rect, 0, 0, 64 * 30]
  origin << [:waitcounter, total_wait += movement_wait]
  origin << [:set_src_rect, 0, 0, 64 * 31]
  origin << [:waitcounter, total_wait += movement_wait]
