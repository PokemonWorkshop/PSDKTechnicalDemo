...
vec4 nausea(vec2 pixPos)
{
	...
	// vec2 clamped = vec2(0.5, 0.5) - clamp(normalizedPosition, vec2(-0.5), vec2(0.5));
	return texture2D(texture, normalizedPosition);
	//create distorsion based on time and position
	// vec2 normal = normalize(vec2(3.0 + (0.5*cos(time * PI + normalizedPosition.y)),3.0 + (0.5*sin(time * PI + normalizedPosition.x))));

	//adjust size
	// normal *= 1.2;

	//multiply coordinates with distorsion and add an offset to recenter
	// vec2 newPos = clamp(pixPos * normal + vec2(0.07,0.05),vec2(0.0),vec2(1.0));

	//final result
	// return texture2D(texture, newPos);
}
