...
vec4 scroll(vec2 pixPos){
  ...
  float dist = clamp(compute_distance(pixPos / v_factor_npot, CENTER) * dist_factor, 0.0, 1.0);
  return vec4(texture2D(extra_texture, mod(pixPos + direction1 * time, extra_texture_factor_npot)).rgb, dist);
}
